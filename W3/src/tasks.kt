...
class LiquidMedicationContainer(name:String, expirationDate: Date, volume:Double, concentration:Int, concentrationUnits:String) :
    MedicationContainer(name, expirationDate) {
    val volume:Double = volume
    val concentration:Int = concentration
    val concentrationUnits:String = concentrationUnits
}

class TabletMedicationContainer(name:String, expirationDate: Date, pillCount: Int, potency:Double, potencyUnits: String) :
    MedicationContainer(name, expirationDate){
    val pillCount: Int = pillCount
    val potency:Double = potency
    val potencyUnits: String = potencyUnits
}

class PharmaceuticalStockTracker{
    var inStockMedications = listOf<MedicationContainer>()
    var count = inStockMedications.size

    fun addContainer(container: MedicationContainer) : Boolean {
        if (!inStockMedications.contains(container)) {
            inStockMedications+=container
            return true
        }
        return false
    }
    fun count(name: String): Int {
        return inStockMedications.count { name == it.name }
    }
}

fun PharmaceuticalStockTracker.removeAllContainers() {
    inStockMedications = listOf()
}
...
fun task5(anyArray: List<Any?>): List<Int> {
    return anyArray.mapNotNull { item ->
        when (item) {
            is Int -> 1
            is Double -> 2
            is String -> 3
...
fun task6(anyArray: List<Any?>): List<Double> {
    return anyArray.mapNotNull { item ->
        when(item) {
            ...
            is Double -> item
            is String -> try { item.toDouble() } catch (e:Exception) { null }
            else -> null
        }
    }
}
